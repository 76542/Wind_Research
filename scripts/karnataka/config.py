"""
scripts/karnataka/config.py
=============================
Karnataka coast: ~300km coastline from Goa border to Kerala border.

Bounding box:
  - Lat: 11.8N (Kerala border) to 14.9N (Goa border)
  - Lon: 73.0E to 75.5E
"""
...
RAW_DATA_DIR = os.path.join(BASE_DIR, 'data', 'raw', 'karnataka')
PROCESSED_DATA_DIR = os.path.join(BASE_DIR, 'data', 'processed', 'karnataka')
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'karnataka')
...
def get_coastline():
    ...
    cache_path = os.path.join(RAW_DATA_DIR, 'karnataka_coastline_cache.csv')
    if os.path.exists(cache_path):
        return pd.read_csv(cache_path)

    print("  [Coastline] Fetching Karnataka coastline from GEE LSIB dataset...")
    try:
        ee.Initialize(project=PROJECT_ID)
    except Exception:
        pass

    LON_MIN, LON_MAX = 74.0, 75.5
    LAT_MIN, LAT_MAX = 11.8, 14.9
