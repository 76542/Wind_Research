"""
scripts/karnataka/ml/finetune.py
=================================
Fine-tune the Maharashtra FT model on Karnataka data.
Same approach as Maharashtra fine-tuning from Gujarat:
  - Load Maharashtra FT weights as starting point
  - Use Gujarat's fitted scaler (never re-fit)
  - 70/15/15 spatial split by point_id
  - LR 1e-4 (same conservative rate)

Usage: python -m scripts.karnataka.ml.finetune
"""
...
OUTPUT_MODEL = os.path.join(PROJECT_ROOT, "models", "mlp_v3_karnataka_finetuned.pth")
...
def main():
    ...
    # ── Load Maharashtra FT model ─────────────────────────────────
    device = torch.device("cpu")
    ckpt = torch.load(MH_MODEL, map_location=device, weights_only=False)
    arch = ckpt["architecture"]
    model = WindSpeedMLPv3(arch["input_dim"], arch["hidden_layers"], arch["dropout"])
    model.load_state_dict(ckpt["model_state_dict"])
    model.to(device)
    print(f"\n  Loaded Maharashtra FT model: {arch['hidden_layers']}")
    ...
    print(f"\n{'='*60}")
    print(f"KARNATAKA FINE-TUNED RESULTS (test set)")
    print(f"{'='*60}")
    ...
    torch.save({
        ...
        "test_rmse": test_rmse,
        "test_r2": test_r2,
        "source": "karnataka_finetuned_from_maharashtra_ft",
    }, OUTPUT_MODEL)
