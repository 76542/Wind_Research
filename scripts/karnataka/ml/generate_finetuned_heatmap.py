"""
scripts/karnataka/ml/generate_finetuned_heatmap.py
====================================================
Runs the Karnataka fine-tuned model and generates coast-adjacent heatmaps.

Key fixes vs original:
  - dist_mask threshold reduced from 0.40 to 0.18 (pulls heatmap to coast)
  - Also runs Gujarat + Maharashtra FT for comparison table
  - Uses Karnataka FT model for the actual heatmap

Usage: python -m scripts.karnataka.ml.generate_finetuned_heatmap
"""
...
KA_COLLOCATED = os.path.join(PROJECT_ROOT, "data", "processed", "karnataka",
                              "karnataka_era5_collocated.csv")
GJ_SCALER = os.path.join(PROJECT_ROOT, "models", "feature_scaler.pkl")
KA_MODEL = os.path.join(PROJECT_ROOT, "models", "mlp_v3_karnataka_finetuned.pth")
...
COAST_PATH = os.path.join(PROJECT_ROOT, "data", "raw", "karnataka",
                           "karnataka_coastline_cache.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", "karnataka")
PREDICTIONS = os.path.join(PROJECT_ROOT, "data", "processed", "karnataka",
...
def main():
    print("\n" + "="*60)
    print("KARNATAKA: Fine-Tuned Heatmap Generation")
    print("="*60)
    ...
    gj_pred = run_model(GJ_MODEL, X, device)
    print("Running Maharashtra FT model...")
    mh_pred = run_model(MH_MODEL, X, device)
    print("Running Karnataka FT model...")
    ka_pred = run_model(KA_MODEL, X, device)
    ...
    gj_m = compute_metrics(y, gj_pred)
    mh_m = compute_metrics(y, mh_pred)
    ka_m = compute_metrics(y, ka_pred)

    results_path = os.path.join(OUTPUT_DIR, "karnataka_finetuned_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        report = f"""
{'='*70}
KARNATAKA RESULTS — ALL THREE MODELS
{'='*70}
Observations: {len(y):,} ({df_raw.point_id.nunique()} points)
ERA5 mean wind: {y.mean():.2f} m/s

                    Gujarat      Maharashtra FT    Karnataka FT
{'='*70}
  RMSE (m/s):       {gj_m['RMSE']:>8.3f}          {mh_m['RMSE']:>8.3f}            {ka_m['RMSE']:>8.3f}
  MAE (m/s):        {gj_m['MAE']:>8.3f}          {mh_m['MAE']:>8.3f}            {ka_m['MAE']:>8.3f}
  R2:               {gj_m['R2']:>8.4f}          {mh_m['R2']:>8.4f}            {ka_m['R2']:>8.4f}
  Bias (m/s):       {gj_m['Bias']:>+8.3f}          {mh_m['Bias']:>+8.3f}            {ka_m['Bias']:>+8.3f}
{'='*70}
"""
        ...
        fmt = "{:<8} {:>5}  {:>9} {:>9} {:>9}  {:>9} {:>9} {:>9}"
        hdr = fmt.format("Bin","N","GJ RMSE","MH RMSE","KA RMSE",
                         "GJ Bias","MH Bias","KA Bias")
        print(hdr); f.write(hdr + "\n")
        for i, label in enumerate(WIND_SPEED_LABELS):
            mask = bi == i; n = mask.sum()
            if n == 0: continue
            gm = compute_metrics(y[mask], gj_pred[mask])
            mm = compute_metrics(y[mask], mh_pred[mask])
            km = compute_metrics(y[mask], ka_pred[mask])
            row = fmt.format(label, n,
                f"{gm['RMSE']:.3f}", f"{mm['RMSE']:.3f}", f"{km['RMSE']:.3f}",
                f"{gm['Bias']:+.3f}", f"{mm['Bias']:+.3f}", f"{km['Bias']:+.3f}")
            print(row); f.write(row + "\n")

    print(f"\nResults: {results_path}")
    df_raw.to_csv(PREDICTIONS, index=False)

    # ── Generate heatmaps (using Karnataka FT) ────────────────────
    print("\nGenerating heatmaps with Karnataka FT model...")
    pred_col = "ka_pred"
    ...
    glon = np.linspace(73.0, 75.5, 500)
