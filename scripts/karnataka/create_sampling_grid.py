"""
scripts/karnataka/create_sampling_grid.py
==========================================
Usage: python -m scripts.karnataka.create_sampling_grid
"""
...
from scripts.karnataka.config import (
...
class SamplingGridGenerator:
    ...
    def generate_grid(self):
        ...
        # Trim to Karnataka boundaries
        grid_df = grid_df[(grid_df['latitude'] >= 11.9) &
                          (grid_df['latitude'] <= 14.85)].copy()
        print(f"  After border trim: {len(grid_df)} points")

        # Exclude Mangalore New Harbour area (~12.85-12.95N, >74.8E)
        grid_df = grid_df[~((grid_df['latitude'] > 12.8) &
                             (grid_df['latitude'] < 13.0) &
                             (grid_df['longitude'] > 74.8))].copy()
        print(f"  After Mangalore harbour exclusion: {len(grid_df)} points")

        grid_df = grid_df.reset_index(drop=True)
        grid_df['point_id'] = [f'{STATE_PREFIX}_{i+1:04d}' for i in range(len(grid_df))]
        return grid_df

    def save_grid(self, grid_df, filename='karnataka_sampling_grid.csv'):
