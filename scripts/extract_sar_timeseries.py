...
from scripts.andhrapradesh import config
...
def main():
    ...
    # Load grid
    grid_file = os.path.join(config.RAW_DATA_DIR, 'andhrapradesh_sampling_grid.csv')
    ...
    if len(timeseries_df) > 0:
        extractor.save_timeseries(timeseries_df, filename='andhrapradesh_sar_timeseries.csv')
        print("\n" + "="*60)
