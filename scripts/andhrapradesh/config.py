"""
scripts/andhrapradesh/config.py
================================
Andhra Pradesh east coast: Bay of Bengal.
Coastline from Tamil Nadu border (~13.5°N) to Odisha border (~19.5°N).

Fine-tuning chain: Gujarat → Tamil Nadu FT → Andhra Pradesh FT
"""
...
RAW_DATA_DIR = os.path.join(BASE_DIR, 'data', 'raw', 'andhrapradesh')
PROCESSED_DATA_DIR = os.path.join(BASE_DIR, 'data', 'processed', 'andhrapradesh')
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'andhrapradesh')
...
def get_coastline():
    ...
    cache_path = os.path.join(RAW_DATA_DIR, 'andhrapradesh_coastline_cache.csv')
    if os.path.exists(cache_path):
        return pd.read_csv(cache_path)

    print("  [Coastline] Fetching Andhra Pradesh coastline from GEE LSIB dataset...")
    try:
