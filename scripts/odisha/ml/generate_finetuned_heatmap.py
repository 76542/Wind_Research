"""
scripts/odisha/ml/generate_finetuned_heatmap.py
Usage: python -m scripts.odisha.ml.generate_finetuned_heatmap
"""
...
OD_COLLOCATED = os.path.join(PROJECT_ROOT, "data", "processed",
                              "odisha", "odisha_era5_collocated.csv")
GJ_SCALER_PATH = os.path.join(PROJECT_ROOT, "models", "feature_scaler.pkl")
FT_MODEL_PATH = os.path.join(PROJECT_ROOT, "models", "mlp_v3_odisha_finetuned.pth")
AP_MODEL_PATH = os.path.join(PROJECT_ROOT, "models", "mlp_v3_andhrapradesh_finetuned.pth")
GJ_MODEL_PATH = os.path.join(PROJECT_ROOT, "models", "mlp_v3_wind_model.pth")
COAST_PATH = os.path.join(PROJECT_ROOT, "data", "raw", "odisha",
                           "odisha_coastline_cache.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", "odisha")
PREDICTIONS_PATH = os.path.join(PROJECT_ROOT, "data", "processed", "odisha",
...
def build_land_mask(coast_pts, grid_lon2d, grid_lat2d):
    """EAST COAST: land is WEST (lon < coast_lon)."""
...
def main():
    ...
    df_raw["ap_pred"] = run_model(AP_MODEL_PATH)
    print("Running Odisha FT model...")
    df_raw["ft_pred"] = run_model(FT_MODEL_PATH)

    y = df_raw[TARGET].values
    gj_m = compute_metrics(y, df_raw["gj_pred"].values)
    ap_m = compute_metrics(y, df_raw["ap_pred"].values)
    od_m = compute_metrics(y, df_raw["ft_pred"].values)

    results_path = os.path.join(OUTPUT_DIR, "odisha_finetuned_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        report = f"""
{'='*70}
ODISHA RESULTS - ALL THREE MODELS
{'='*70}
Observations: {len(y):,} ({df_raw.point_id.nunique()} points)
ERA5 mean wind: {y.mean():.2f} m/s

                    Gujarat      AP FT             Odisha FT
{'='*70}
  RMSE (m/s):       {gj_m['RMSE']:>8.3f}          {ap_m['RMSE']:>8.3f}            {od_m['RMSE']:>8.3f}
  MAE (m/s):        {gj_m['MAE']:>8.3f}          {ap_m['MAE']:>8.3f}            {od_m['MAE']:>8.3f}
  R2:               {gj_m['R2']:>8.4f}          {ap_m['R2']:>8.4f}            {od_m['R2']:>8.4f}
  Bias (m/s):       {gj_m['Bias']:>+8.3f}          {ap_m['Bias']:>+8.3f}            {od_m['Bias']:>+8.3f}
{'='*70}
"""
        print(report); f.write(report)
        bi = np.clip(np.digitize(y, WIND_SPEED_BINS)-1, 0, len(WIND_SPEED_LABELS)-1)
        fmt = "{:<8} {:>5}  {:>9} {:>9} {:>9}  {:>9} {:>9} {:>9}"
        hdr = fmt.format("Bin","N","GJ RMSE","AP RMSE","OD RMSE","GJ Bias","AP Bias","OD Bias")
        ...
        for i, label in enumerate(WIND_SPEED_LABELS):
            ...
            am = compute_metrics(y[mask], df_raw["ap_pred"].values[mask])
            om = compute_metrics(y[mask], df_raw["ft_pred"].values[mask])
            row = fmt.format(label, n, f"{gm['RMSE']:.3f}", f"{am['RMSE']:.3f}", f"{om['RMSE']:.3f}",
                           f"{gm['Bias']:+.3f}", f"{am['Bias']:+.3f}", f"{om['Bias']:+.3f}")
    ...
    grid_lat = np.linspace(18.9, 22.1, 400)
