"""
scripts/odisha/create_sampling_grid.py
Usage: python -m scripts.odisha.create_sampling_grid
"""
...
from scripts.odisha.config import (
...
class SamplingGridGenerator:
    ...
    def _seaward_direction(self, pts, idx):
        ...
        # Land reference: interior Odisha
        land_lon, land_lat = 83.0, 21.0
        lon0, lat0 = pts[idx]
        vtl = (land_lon - lon0, land_lat - lat0)
        mag_l = np.sqrt(vtl[0]**2 + vtl[1]**2)
        if mag_l < 1e-10: return perp_a
    ...
    def generate_grid(self):
        ...
        print(f"\nStep 3 - Projecting offshore (into Bay of Bengal)...")
        raw_points = []
        pid = 1
        ...
        # Trim to Odisha boundaries
        grid_df = grid_df[(grid_df['latitude'] >= 19.1) &
                          (grid_df['latitude'] <= 21.9)].copy()
        print(f"  After border trim: {len(grid_df)} points")

        # Exclude Chilika Lake area (~19.6-19.8N, nearshore lagoon)
        grid_df = grid_df[~((grid_df['latitude'] > 19.55) &
                             (grid_df['latitude'] < 19.85) &
                             (grid_df['offshore_distance_km'] <= 20) &
                             (grid_df['longitude'] < 85.5))].copy()
        print(f"  After Chilika Lake exclusion: {len(grid_df)} points")

        # Exclude Paradip port area
        grid_df = grid_df[~((grid_df['latitude'] > 20.2) &
                             (grid_df['latitude'] < 20.35) &
                             (grid_df['offshore_distance_km'] <= 20))].copy()
        print(f"  After Paradip port exclusion: {len(grid_df)} points")

        grid_df = grid_df.reset_index(drop=True)
        grid_df['point_id'] = [f'{STATE_PREFIX}_{i+1:04d}' for i in range(len(grid_df))]
        return grid_df

    def save_grid(self, grid_df, filename='odisha_sampling_grid.csv'):
...
def main():
    print(f"{'='*60}\n{STATE_NAME} Coast - Sampling Grid Generator\n{'='*60}\n")
