...
RHO = 1.16
# Shared colour scale for the stitched map (west-coast WPD tops out ~193 W/m²)
VMAX = 200
...
def main():
    ...
    def make_map(cols, title_prefix, filename, subtitle):
        if HAS_CARTOPY:
            ...
            fig, axes = plt.subplots(1, 1, figsize=(9, 13),
        ...
        for ax, (col, label) in zip(axes, cols):
            ...
            plt.colorbar(im, ax=ax, label='Wind Power Density (W/m²)', shrink=0.6,
                         pad=0.02)

        fig.suptitle(
            f'Offshore Wind Power Density — Indian West Coast (2020-2024)\n'
            f'{subtitle}',
    ...
    make_map(
        [('wpd', 'WPD (W/m²)')],
        'MLP v3 (SAR)', 'west_coast_wpd_heatmap.png',
...
"""
        ...
        fmt = "{:<15} {:>6} {:>10} {:>9} {:>9}  {:>10}"
        hdr = fmt.format("State", "Points", "WPD mean", "WPD min", "WPD max", "Model")
        f.write(hdr + "\n")
        print(hdr)
        f.write("-" * 65 + "\n")
        ...
        for state in ['Gujarat', 'Maharashtra', 'Goa', 'Karnataka', 'Kerala']:
            ...
            row = fmt.format(state, len(s),
                ...
                f"{s.wpd.max():.0f}", model_names[state])
            f.write(row + "\n")
            print(row)
