"""
scripts/tamilnadu/create_sampling_grid.py
==========================================
Key difference from west coast: sea is EAST, land is WEST.
The seaward direction is reversed.

Usage: python -m scripts.tamilnadu.create_sampling_grid
"""
...
from scripts.tamilnadu.config import (
...
class SamplingGridGenerator:
    ...
    def _seaward_direction(self, pts, idx):
        """Compute seaward (offshore) direction.
        Tamil Nadu east coast: land is WEST, sea is EAST.
        We use a land reference point inland (e.g., 76°E, 11°N = Western Ghats)
        and pick the perpendicular pointing AWAY from it.
        """
        n = len(pts)
        ...
        # Land reference: interior Tamil Nadu (Western Ghats side)
        land_lon, land_lat = 76.0, 11.0
        lon0, lat0 = pts[idx]
        ...
        # Pick perpendicular pointing AWAY from land
    ...
    def generate_grid(self):
        ...
        # Trim to Tamil Nadu east coast boundaries
        grid_df = grid_df[(grid_df['latitude'] >= 8.1) &
                          (grid_df['latitude'] <= 13.5)].copy()
        print(f"  After border trim: {len(grid_df)} points")

        # Exclude Chennai harbour area (~13.0-13.2N, >80.2E nearshore)
        grid_df = grid_df[~((grid_df['latitude'] > 13.0) &
                             (grid_df['latitude'] < 13.2) &
                             (grid_df['longitude'] > 80.25) &
                             (grid_df['offshore_distance_km'] <= 20))].copy()
        print(f"  After Chennai harbour exclusion: {len(grid_df)} points")

        # Exclude Palk Strait / Rameshwaram bridge area
        # (shallow, not representative of open ocean)
        grid_df = grid_df[~((grid_df['latitude'] > 9.2) &
                             (grid_df['latitude'] < 9.4) &
                             (grid_df['longitude'] > 79.3))].copy()
        print(f"  After Palk Strait exclusion: {len(grid_df)} points")

        grid_df = grid_df.reset_index(drop=True)
        grid_df['point_id'] = [f'{STATE_PREFIX}_{i+1:04d}' for i in range(len(grid_df))]
        return grid_df

    def save_grid(self, grid_df, filename='tamilnadu_sampling_grid.csv'):
