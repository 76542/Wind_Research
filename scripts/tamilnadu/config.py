"""
scripts/tamilnadu/config.py
=============================
Tamil Nadu east coast: Bay of Bengal side.
Coastline from Kanyakumari (~8°N) to AP border (~13.5°N).

Key differences from west coast:
  - Land is WEST, sea is EAST (reversed from west coast)
  - Bay of Bengal: NE monsoon dominant, cyclone-prone
  - Wider continental shelf off Tamil Nadu
"""
...
RAW_DATA_DIR = os.path.join(BASE_DIR, 'data', 'raw', 'tamilnadu')
PROCESSED_DATA_DIR = os.path.join(BASE_DIR, 'data', 'processed', 'tamilnadu')
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'tamilnadu')
...
def get_coastline():
    ...
    cache_path = os.path.join(RAW_DATA_DIR, 'tamilnadu_coastline_cache.csv')
    if os.path.exists(cache_path):
        return pd.read_csv(cache_path)

    print("  [Coastline] Fetching Tamil Nadu coastline from GEE LSIB dataset...")
    try:
        ee.Initialize(project=PROJECT_ID)
    except Exception:
        pass

    # Tamil Nadu east coast bounding box
