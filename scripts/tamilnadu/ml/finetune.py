"""
scripts/tamilnadu/ml/finetune.py
=================================
Fine-tune from GUJARAT ORIGINAL weights (not Kerala/Karnataka FT).

Rationale: The west coast chain (GJ→MH→KA→KL) has progressively
specialized spatial features for Arabian Sea. For Bay of Bengal,
Gujarat's general SAR-wind representations are a better starting
point than the overspecialized west coast chain.

Usage: python -m scripts.tamilnadu.ml.finetune
"""
...
TN_COLLOCATED = os.path.join(PROJECT_ROOT, "data", "processed", "tamilnadu",
                              "tamilnadu_era5_collocated.csv")
GJ_SCALER = os.path.join(PROJECT_ROOT, "models", "feature_scaler.pkl")
# KEY: Fine-tune from Gujarat ORIGINAL, not west coast chain
GJ_MODEL = os.path.join(PROJECT_ROOT, "models", "mlp_v3_wind_model.pth")
OUTPUT_MODEL = os.path.join(PROJECT_ROOT, "models", "mlp_v3_tamilnadu_finetuned.pth")
...
def main():
    print("\n" + "="*60)
    print("TAMIL NADU FINE-TUNING (from Gujarat original)")
    print("First east coast state — Bay of Bengal")
    print("="*60)
    ...
    # ── Load Gujarat ORIGINAL model ───────────────────────────────
    device = torch.device("cpu")
    ckpt = torch.load(GJ_MODEL, map_location=device, weights_only=False)
    arch = ckpt["architecture"]
    model = WindSpeedMLPv3(arch["input_dim"], arch["hidden_layers"], arch["dropout"])
    model.load_state_dict(ckpt["model_state_dict"])
    model.to(device)
    print(f"\n  Loaded Gujarat ORIGINAL model: {arch['hidden_layers']}")
    print(f"  (NOT using west coast chain — starting from general SAR features)")
    ...
    print(f"\n{'='*60}")
    print(f"TAMIL NADU FINE-TUNED RESULTS (test set)")
    print(f"{'='*60}")
    ...
    torch.save({
        ...
        "test_rmse": test_rmse,
        "test_r2": test_r2,
        "source": "tamilnadu_finetuned_from_gujarat_original",
    }, OUTPUT_MODEL)
