"""
scripts/goa/collocate_era5_sar.py

Usage:
  cd Wind_Research
  python -m scripts.goa.collocate_era5_sar
"""
...
GRID_FILE = os.path.join(BASE_DIR, "data", "raw", "goa",
                         "goa_sampling_grid.csv")
ERA5_DIR = Path(os.path.join(BASE_DIR, "data", "raw", "goa", "era5_downloads"))
OUTPUT_FILE = os.path.join(BASE_DIR, "data", "processed", "goa",
...
if missing_coords > 0:
    print(f"  WARNING: {missing_coords} rows have no coordinates")

print(f"SAR data: {len(sar):,} rows, {sar['point_id'].nunique()} unique points")

era5_files = sorted(ERA5_DIR.glob("era5_goa_*.nc"))
if not era5_files:
...
print(f"  ERA5 grid: {len(era5_lats)} lats x {len(era5_lons)} lons, "
