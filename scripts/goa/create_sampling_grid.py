"""
scripts/goa/create_sampling_grid.py
=====================================
Generate offshore sampling grid for Goa coast.

Goa is tiny (~105km coastline) so expect ~25-35 points total.
No major exclusions needed — no large harbours or inland water bodies.

Usage:
  cd Wind_Research
  python -m scripts.goa.create_sampling_grid
"""
...
from scripts.goa.config import (
...
class SamplingGridGenerator:
    ...
    def generate_grid(self):
        ...
        # Trim to Goa boundaries (avoid spilling into Maharashtra/Karnataka)
        grid_df = grid_df[
            (grid_df['latitude'] >= 14.85) &
            (grid_df['latitude'] <= 15.75)
        ].copy()
        print(f"  After border trim: {len(grid_df)} points remain")
    ...
    def save_grid(self, grid_df, filename='goa_sampling_grid.csv'):
