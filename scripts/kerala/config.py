"""
scripts/kerala/config.py
=========================
Kerala coast: ~590km coastline from Karnataka border to Kanyakumari.

Bounding box:
  - Lat: 8.0N (Kanyakumari) to 12.0N (Karnataka border)
  - Lon: 74.0E to 77.5E
"""
...
RAW_DATA_DIR = os.path.join(BASE_DIR, 'data', 'raw', 'kerala')
PROCESSED_DATA_DIR = os.path.join(BASE_DIR, 'data', 'processed', 'kerala')
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'kerala')
...
def get_coastline():
    ...
    cache_path = os.path.join(RAW_DATA_DIR, 'kerala_coastline_cache.csv')
    if os.path.exists(cache_path):
        return pd.read_csv(cache_path)

    print("  [Coastline] Fetching Kerala coastline from GEE LSIB dataset...")
    try:
