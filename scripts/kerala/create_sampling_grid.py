"""
scripts/kerala/create_sampling_grid.py
Usage: python -m scripts.kerala.create_sampling_grid
"""
...
from scripts.kerala.config import (
...
class SamplingGridGenerator:
    ...
    def _seaward_direction(self, pts, idx):
        ...
        # Kerala: land is EAST, sea is WEST
        land_lon, land_lat = 78.0, 10.0
        lon0, lat0 = pts[idx]
    ...
    def generate_grid(self):
        ...
        # Trim to Kerala boundaries
        grid_df = grid_df[(grid_df['latitude'] >= 8.1) &
                          (grid_df['latitude'] <= 11.95)].copy()
        print(f"  After border trim: {len(grid_df)} points")

        # Exclude Kochi harbour area (~9.9-10.0N, >76.2E)
        grid_df = grid_df[~((grid_df['latitude'] > 9.9) &
                             (grid_df['latitude'] < 10.05) &
                             (grid_df['longitude'] > 76.2))].copy()
        print(f"  After Kochi harbour exclusion: {len(grid_df)} points")

        grid_df = grid_df.reset_index(drop=True)
        grid_df['point_id'] = [f'{STATE_PREFIX}_{i+1:04d}' for i in range(len(grid_df))]
        return grid_df

    def save_grid(self, grid_df, filename='kerala_sampling_grid.csv'):
