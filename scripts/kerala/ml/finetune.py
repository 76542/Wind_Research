"""
scripts/kerala/ml/finetune.py
==============================
Fine-tune the Karnataka FT model on Kerala data.
Chain: Gujarat -> Maharashtra FT -> Karnataka FT -> Kerala FT

Usage: python -m scripts.kerala.ml.finetune
"""
...
OUTPUT_MODEL = os.path.join(PROJECT_ROOT, "models", "mlp_v3_kerala_finetuned.pth")
...
def main():
    ...
    # ── Load Karnataka FT model ───────────────────────────────────
    device = torch.device("cpu")
    ckpt = torch.load(KA_MODEL, map_location=device, weights_only=False)
    arch = ckpt["architecture"]
    model = WindSpeedMLPv3(arch["input_dim"], arch["hidden_layers"], arch["dropout"])
    model.load_state_dict(ckpt["model_state_dict"])
    model.to(device)
    print(f"\n  Loaded Karnataka FT model: {arch['hidden_layers']}")
    ...
    print(f"\n{'='*60}")
    print(f"KERALA FINE-TUNED RESULTS (test set)")
    print(f"{'='*60}")
    ...
    torch.save({
        ...
        "test_rmse": test_rmse,
        "test_r2": test_r2,
        "source": "kerala_finetuned_from_karnataka_ft",
    }, OUTPUT_MODEL)
